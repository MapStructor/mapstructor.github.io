...
(function () {
  ...
  function transformIndexHtml(src, variant, hasRasters) {
    ...
    // the saved stacking order travels with the copy too — same trick, and unconditional: the
    // catch-all ../platform/ strip below would otherwise delete the module from the standalone
    html = html.replace(/^[ 	]*<script src="\.\.\/platform\/layerOrder\.js[^"]*"[^>]*><\/script>.*$/m,
      "    <script src=\"project/layerOrder.js\"></script>");
  ...
  async function buildZip(opts) {
    ...
    if (hasRasters) {
      ...
      for (var ri = 0; ri < rasterItems.length; ri++) {
        ...
        // `thin` rides along too: the standalone loader has no layers table to read `type` from, and
        // without it a line layer's raster picks the coarse level and draws fat (8/18)
        rasterData.push({ lid: it.lid, slug: it.slug || null, cfg: { bounds: it.cfg.bounds, yearBase: it.cfg.yearBase, bakedAt: it.cfg.bakedAt, levels: levels, indexed: it.cfg.indexed || undefined, lut: lut, isBorder: it.isBorder || undefined, fillLid: it.fillLid || undefined, thin: it.thin || undefined } });
      }
    ...
    // 2c. the explicit draw order, frozen with the map — it is part of how the map LOOKS, so the
    // copy must carry it. NL avoids backslash escapes in this generated source entirely.
    try {
      var lo = window.MSLayerOrder;
      var NL = String.fromCharCode(10);
      var loJs = "/* generated by MapStructor - the map's saved layer order */" + NL +
        "window.__msLayerOrder = " + JSON.stringify(lo ? lo.order() : []) + ";" + NL +
        "window.__msLabelsOnTop = " + ((lo ? lo.labelsOnTop() : true) ? "true" : "false") + ";" + NL + NL +
        await fetchText("../platform/layerOrder.js");
      zip.file("map/project/layerOrder.js", loJs);
    } catch (eLO) { setStatus("Could not embed the layer order - the copy will use the sidebar order"); }
